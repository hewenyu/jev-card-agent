#!/bin/sh
# Run manually from the deployment directory. Never replace a bot mid-hand.
set -eu
...
async function request(path, method = 'GET') {
  ...
  if (!response.ok) throw new Error(`Drain API returned HTTP ${response.status}; container left unchanged`);
