...
import { FactsStore, emptyFactsSnapshot, factsHash } from './store.js';
...
const JSON_CONTEXT = "CASE WHEN json_valid(context) THEN context ELSE '{}' END";
const JSON_PROPOSAL = "CASE WHEN json_valid(proposal) THEN proposal ELSE '{}' END";
const AUDIT_ELIGIBLE = `(json_type(${JSON_CONTEXT},'$.framework')='object' OR json_type(${JSON_CONTEXT},'$.knowledge')='object' OR json_type(${JSON_PROPOSAL},'$.framework')='object')`;
/** Runs only in the isolated worker (exported for deterministic integration tests). */
export class FactsWorker {
  ...
  constructor(
    ...
    if (
      ...
      throw new Error('Facts database must be separate from raw history');
    ...
    this.facts = new FactsStore(derivedPath);
    initializeOpponentMemory(this.facts.db, false);
    this.facts.db.exec(`CREATE TABLE IF NOT EXISTS opponent_summaries (
  ...
  tick(now = new Date().toISOString()): SlowLoopStatus {
    ...
    const previous = this.facts.cursor(EVENT_CURSOR);
    const summarized = this.facts.cursor(SUMMARY_CURSOR);
    const watermark =
      summarized < previous
        ? previous
        : syncOpponentMemory(this.raw, this.facts.db, this.batchSize, now);
    if (watermark > summarized) this.summarize(summarized, watermark, now);
    if (watermark > this.facts.latest().evidenceEventId) {
      const opponents = this.facts.db
        .prepare(
      ...
      const content: Omit<KnowledgeSnapshot, 'contentHash'> = {
        ...
        version: `poker-facts-v1-e${watermark}`,
        ...
        validation: [
          ...
          'Pure deterministic public evidence; no strategy cards, advice or strategy publication.',
        ],
      };
      this.facts.appendSnapshot({ ...content, contentHash: factsHash(content) });
    }
    const upper = Number(
    ...
    const rows = this.raw
      ...
      .all(this.facts.cursor(AUDIT_CURSOR), upper, this.batchSize);
    for (const row of rows) {
      ...
      this.facts.advance(AUDIT_CURSOR, Number(row.cursor));
    }
    if (rows.length < this.batchSize) this.facts.advance(AUDIT_CURSOR, upper);
    return this.status(new Date().toISOString());
  }
  private summarize(after: number, through: number, asOf: string): void {
    const names = this.facts.db
      .prepare(
    ...
    const opponents = getOpponentMemory(this.facts.db, state, asOf, false);
    this.facts.db.exec('SAVEPOINT summary_batch');
    try {
      const insert = this.facts.db
      ...
      this.facts.advance(SUMMARY_CURSOR, through);
      this.facts.db.exec('RELEASE summary_batch');
    } catch (error) {
      this.facts.db.exec('ROLLBACK TO summary_batch; RELEASE summary_batch');
      throw error;
    }
  }
  status(lastCompletedAt: string | null = null): SlowLoopStatus {
    const eventCursor = this.facts.cursor(EVENT_CURSOR);
    const decisionCursor = this.facts.cursor(AUDIT_CURSOR);
    return {
      enabled: true,
      running: true,
      lastCompletedAt,
      ...
      latestVersion: this.facts.latest().version,
      error: null,
    };
  }
  close(): void {
