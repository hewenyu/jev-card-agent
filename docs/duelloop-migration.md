...
The manifest records snapshot SHA-256, SQLite `quick_check`, table row counts, preserved pending-action/decision-block counts, and legacy reader validation. It contains no environment values, prompts, cards or decision contents. `.env.next` preserves current credentials and research settings using literal single-quoted values where possible so Compose cannot expand `$` inside secrets. Values containing an apostrophe use double quotes only when they contain no dollar sign, double quote or backslash; values outside this common Node/Compose-safe subset fail migration instead of changing credentials. Node dotenv round-trip validation is required, and a synthetic credential regression checks Compose parsing when its CLI is installed. It first migrates `DEEPSEEK_API_KEY`, `DEEPSEEK_MODEL` and `DEEPSEEK_BASE_URL` to the corresponding `DUELLOOP_RESEARCH_*` settings without replacing nonempty explicit values. It does not reuse the offline-only `DEEPSEEK_API_BASE_URL` or `REASONING_API_KEY`. It then removes `ASYNC_LLM_*`, `LLM_ADVICE_*`, `LLM_RESEARCH_*`, all `REASONING_*` and `DEEPSEEK_*` settings, and `HYBRID_TIMEOUT_MS`, and disables bot/research autostart. Keep a separate private environment for any legacy evaluation experiments. `FACTS_ENABLED` preserves the prior explicit value or inherits `RESEARCH_ENABLED`. Legacy stores remain accessible through read-only history readers. Review and configure fresh development/final protocols before enabling research.
