...
`v2.0.4` 已于 2026-09-26 上线，使用正式 DuelLoop SDK `v0.2.3`，包含 Jev HTTP 403
重试与显式模型身份迁移。[最新部署验收](deployment-acceptance-v2.0.4.md)记录镜像、
备份清理及维护期间的租约停牌与恢复；最终验收状态以该报告为准。
[v2.0.3 验收](deployment-acceptance-v2.0.3.md)保留此前配置清理和研究接口迁移，
[v2.0.1 验收](deployment-acceptance-v2.0.1.md)保留首次迁移与数据恢复记录。
...
`manage.sh update` 只负责排空、拉取和重建，不调用 `migrate:duelloop`，也不自动
改写 `.env`。升级前必须完成模型变量迁移并核对正式研究凭据。

### v2.0.3 → v2.0.4 的 Jev 重试身份迁移

HTTP 403 重试改变模型行为摘要；已有 v2.0.3 bootstrap release 需要显式维护重绑。
先暂停研究并等待或取消当前研究任务，安全停止 Bot、确认离桌，再停止全部应用
写进程；创建并验证包含数据库、WAL、协议和原配置的完整备份。使用待部署的
v2.0.4 镜像、既有私有配置与数据挂载执行以下 CLI，不启动正常服务：

```sh
docker compose run --rm --no-deps app node dist/cli/rebind-jev-retry.js \
  --expected-release "$PREVIOUS_RELEASE_DIGEST" --evidence "$REPORT_PATH_IN_CONTAINER"
docker compose run --rm --no-deps app node dist/cli/rebind-jev-retry.js \
  --expected-release "$PREVIOUS_RELEASE_DIGEST" --evidence "$REPORT_PATH_IN_CONTAINER" --apply
```

`PREVIOUS_RELEASE_DIGEST` 是已核对的当前旧 release；报告路径必须指向本次修改
实际运行且全部通过的 Vitest JSON 报告，包含 CLI 要求的重试与维护回归套件。
保留报告 SHA-256 与对应源码提交，不能用手写成功摘要代替。第一条默认只读
dry-run，核对旧/新依赖及策略摘要后再执行 `--apply`。操作只接受原有未变化的
bootstrap 策略，保留旧 release、历史手绑定和持久停牌，不声称研究验证通过。

CLI 失败后保持服务停止并先检查 active release：SDK 提交后的最终核对也可能
失败，退出码非零不代表重绑未生效。恢复 Bot 前仍需独立验收并显式解除原停牌。
回滚到 v2.0.3 必须使用维护前已验证的完整备份与旧镜像，在独立目录恢复；不能
让旧模型直接继续使用已重绑的工作库。实际执行与验收结果见
[v2.0.4 部署验收](deployment-acceptance-v2.0.4.md)。

**生产空间限制：** 2026-09-26 的 v2.0.3 更新前约剩 11 GB 空间，原始主库约
11.67 GB，不足以容纳默认备份流程的临时快照及保守压缩空间预留。当时使用停机
流式归档，生成约 2.70 GB 的完整归档；当时保留三个批次是历史状态，当前保留
口径见下文。每次维护重新核对容量；不要在空间不足时直接执行下面的默认
`backup`，它会先排空 Bot，再因容量检查失败保持停止。既有回滚归档不包含之后
新增的牌局。
...
备份逐库执行 `quick_check`，流式压缩为 `.sqlite.gz`，再解压校验 SHA-256
一致后才删除本次临时未压缩快照。Compose 复制到宿主 `data/backups/` 后，
再次比较归档 SHA-256，成功才删除容器内本次生成的临时归档；旧备份不自动
删除。这样不会永久保留同一备份的容器、宿主两份副本。归档恢复时先执行
`gzip -dc BACKUP.sqlite.gz > RESTORE.sqlite`，再用 SQLite 检查、隔离验证。

每库写入前使用 `statfs` 检查临时快照和最坏压缩体积，并保留至少 1 GiB
可用空间；宿主复制前也检查目标空间。检查不能防止其他进程同时写满磁盘。
空间不足、压缩损坏或复制校验失败时操作终止，保留已有备份和诊断文件，
Bot 与研究保持已安全停下的状态。释放或扩充空间、确认现场后重新备份，
验证部署完成后再显式恢复运行，不能把失败的备份当作可用回滚点。
...
备份保留口径为最近 **一个完整且已验证的批次**，同批数据库、WAL、协议及恢复
配置作为整体处理，不能按单个文件数量截断。新批次确认完整并通过哈希及归档
完整性检查后，再清理更早批次；新备份失败时保留原有可用批次。私有
`data/backups/retention.json` 记录实际位置和校验值，清理前核对它们不属于当前
工作挂载。该口径不表示已经配置定时自动删除；当前工作库与演练工作区不计为
备份批次。

2026-09-25 曾保留当天上线前、v1.4.3 前及 v1.4.2 发布备份，v2.0.3 更新时曾
调整为该次上线前、v2.0.2 前及旧生产停机归档。这些列表仅说明历史操作，不能
作为当前可用备份清单；实际保留情况以最新私有清单和归档复验为准。

v2.0.4 维护期间，旧归档核验清理与 API 延迟、15 秒运行租约过期同时出现；
现有证据没有确定磁盘 I/O 是根因。今后大量归档全量校验、散列和清理优先安排
在 Bot 与研究停止写入的窗口；必须在线执行时选择低负载时段、限制维护资源，
监测健康响应与租约剩余时间，避免维护任务影响租约续期。租约过期停牌后先确认
官方离桌及未决执行状态，再使用正式恢复入口，不通过直接改库绕过保护。
...
历史上，2026-09-25 的旧生产主库、WAL 和配置曾保存到逐文件 SHA-256 校验的
私有压缩归档，彼时确认服务器与本地各一份；旧卷中的六个原始主库/WAL 文件
在核对归档与新工作副本后移除。该归档可能随保留策略清理，不保证现在仍存在。
**不能直接启动旧 named volume 回滚。** 若确需恢复旧 `1.4.3`，必须先确认对应
`stopped-production.tar.gz` 和私有清单仍可用且校验通过，将配套主库和 WAL
恢复到新的可写目录，再匹配旧镜像、原配置与路径映射。归档本身不得作为可写
数据挂载；先隔离核对 SQLite 与历史 HTTP 读取，再切换。
