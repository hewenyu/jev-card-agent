...
**当前生产空间限制：** 2026-09-25 恢复后约有 15 GiB 空闲，当天按三个版本
清理旧备份后增至约 19 GiB；但约 9.675 GiB 的
...
备份保留口径为最近 **三个完整批次**，同批数据库、WAL、协议及恢复配置作为
整体处理，不能按单个文件数量截断。2026-09-25 已人工清理更早的备份，保留
当天上线前快照、9 月 24 日 v1.4.3 前快照及 v1.4.2 发布备份，逐份通过哈希及
相应归档完整性检查。私有 `data/backups/retention.json` 记录实际位置和校验值；
其中 v1.4.2 已移出退役 Docker 卷。此次没有配置定时自动删除；今后产生新备份后，
应先验证完整性，再按三个批次执行保留策略。当前工作库与演练工作区不计为备份批次。
