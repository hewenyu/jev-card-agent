...
公共场当前行动窗口为 45 秒；重连不重置它。冷恢复剩余时间未知时不重新给予完整期限，记录失败并停牌。纯 Jev 单次调用默认 10 秒、整体决策 40 秒；组合策略使用独立分析与全链路期限，均须为提交留出时间。
...
上线验收检查新 Run 的 strategy、成功 Jev attempts、行动接受、历史留存与仅 Jev 的费用记录。数小时是采集目标，开始运行不等于已采足；历史 fallback 样本和新版本失败样本必须单独标记。后续复盘先固定样本及版本，再设计 harness 对照，不能将历史实际收益当成替代动作收益。详见[评估说明](evaluation.md)。
