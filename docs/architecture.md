...
本轮部署目标为 `BOT_STRATEGY=jev`，由 Jev 直接根据可见局面、同手 session、对手统计及历史摘要选择合法候选。持续收集真实牌局与决策数据。本次按所有者要求先归档并清理旧牌局历史，保留费用账本；新版从空记录开始采样，不在采集期间自动改写策略。DeepSeek 与标准推理 provider 保留为显式 `jev-reasoning` 实验选项；组合策略默认 `REASONING_MODE=always`，旧的 Jev 按需路由仅在显式配置 `adaptive` 时使用。纯 Jev 不因环境中保留了分析模型密钥而调用分析服务。
...
显式启用的组合策略在每个具有有效行动授权的决策回合请求推理分析，再由 Jev 在冻结的合法候选中作最终选择。推理模型只提供建议，不能直接提交行动。组合策略默认使用 always 分析模式；旧的 Jev 按需路由保留为明确的后台可选配置，不由网页修改。超时、预算不足、恢复回合剩余时间未知等故障仍遵守现有合法 fallback 与不重复下注合同；公开记录明确标记未成功分析，不能宣称每次都完成推理。
...
上一版本根据比较使用 `deepseek-flash`、`thinking=disabled`，分析单次 10 秒、Hybrid 总计 40 秒；本轮切换纯 Jev，保留这些配置用于将来显式对照。GPT 与 Claude 不作为自动兜底；DeepSeek 分析失败只进入 Jev 剩余时间决策或合法本地 fallback。历史集成与本次纯 Jev 上线证据分别记录于验证报告，不把配置选定当作上线成功。比较记录保留原协议、模式、deadline、输出上限、模型身份与费用。
...
Overview 只展示战绩：净收益、盈利手牌胜率、赛季积分，以及可切换的积分/累计净收益曲线。账户明细、补筹事件、Agent 资料、模型费用与最近手牌列表不放在 Overview；账户及资金事件继续在 Live 展示，逐手复盘在 Replay 展示。保留现有 Run 选择器，所有历史统计和曲线明确属于所选 Run，Demo 单独标识。

净收益累加所选 Run 全部已完成且收益可核实的手牌，不受历史分页限制，也不含买入、rebuy。胜率为净收益大于零的手牌数 / 已核实手牌数，平局计入分母；无有效样本显示未知，不显示 0%。bb/100 不叫百分比胜率。赛季积分使用同次官方 REST 账户快照的 `available_after + chips_at_table`，仅取已保存的 `balance_sync` 且两项均已知的记录，不能混入 WebSocket 座位余额或用补筹额度推算。积分包含补筹，因此不等同扑克盈利；尚无快照时留空，历史 Run 显示最后记录值及时间。
...
本次以稳定策略持续采样为目标：使用持久 SQLite 卷，自动参赛与 rebuy，按手牌边界更新。本次按所有者授权在备份后清理旧 Run、牌局、决策、资金展示记录与恢复检查点；模型费用账本及未知费用预留保留，新版采集的数据持续保存。每次决策留存当时可见状态、完整已观察本手行动、候选、对手样本与分母、session、近期已核实结果摘要、信息截止序号及策略版本。成功 Jev 调用保存实际请求与 schema 解析后的响应、概率、usage、耗时及 attempts；失败调用保存状态及可用诊断，不宣称保留逐字失败响应。

输入历史有界不意味着删除数据库历史。原始 WS 事件、动作 prepared/sent/accepted/rejected 状态、完整 hand_result 和可核实收益继续留存；Replay 恢复快照后的重放事件按服务端序号去重，避免重复累计下注。下注/结算输入口径修正升级为 `visible-context-v4`，新旧采样可按 Run、代码 revision 和版本区分。纯 Jev 实时文案显示 Jev 选择及行动，不伪造额外分析或思考文本。

上线验收检查新 Run 的 strategy、成功 Jev attempts、行动接受、历史留存与仅 Jev 的费用记录。数小时是采集目标，开始运行不等于已采足；费用预算耗尽或 fallback 样本必须单独标记。后续复盘先固定样本及版本，再设计 harness 对照，不能将历史实际收益当成替代动作收益。详见[评估说明](evaluation.md)。
