...
完整原始事件、行动、结果和模型请求保存在私有 SQLite，摘要只是受限的模型输入，不替代长期历史保存。`PUBLIC_HISTORY` 开放匿名公共观战与已结束牌局的脱敏回放；实时私有底牌保留在后端，管理通过 Compose、CLI 或受保护的内部 API 执行。公开 Demo 继续使用合成数据，原始数据库不提交到仓库。
...
| Live              | SSE 公共牌局、最近行动、筹码移动与派奖动画，自动更新和重连     |
| Replay / Decision | 街道与行动时间线、当时状态、对手样本、候选分布、执行确认及结算 |
| Experiments       | 已保存且允许公开的 Run 比较、重跑报告、版本与样本覆盖率        |
...
Live 页面展示实时公共牌、座位、行动玩家、筹码、底池与公开行动流，通过同源 SSE 持续接收后端的只读观战快照，并自动重连。后台状态的变更不依赖页面是否打开。进行中的私有底牌、合法行动授权 token、模型请求和待完成决策不进入公开实时流；已结束牌局继续提供完整已记录历史及决策复盘。
