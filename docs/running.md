...
本机最新部署入口记录在私有 `data/maintenance/v2.0.4/DEPLOYMENT.md`，包含 SSH
别名、部署目录、实际挂载和本次验证文件位置。维护前先查这份记录并核对现场；
公开结果见 [v2.0.4 部署验收](deployment-acceptance-v2.0.4.md)。
...
本手进行中新增入座的玩家不参与已经开始的手牌：`player_joined` 缺少 `in_hand` 时应记录为本手未参与；服务端显式提供参与状态时仍以服务端为准。同一玩家的重复入座通知保留已知参与状态，下一手重新根据新手事件和快照确定参与者。这样随后 `table_state` 补齐 `in_hand=false` 不会虚构一次决策状态变化，也不会把等待下一手的玩家送入 Jev 的参战对手信息。验证和上线记录见 [verification.md](verification.md)。

上述座位语义修正不能放宽有效行动校验：牌桌、手牌、行动授权、底池、牌面、合法金额或真实参战玩家变化时，旧决策仍不得提交。模型调用重试与提交前状态校验分开记录；Jev 成功返回后被 Runtime 拒绝，不应解释为供应商调用失败。更新前备份并保留全部原始事件与决策，旧失败记录不改写。
...
积分同步修复合同：直接保存 `/api/season/me` 的 `score` 与 `season_id`，不以余额相加代替官方积分。首次启动、每次入桌前、入桌后及确认离桌后分别记录官方快照；相同余额也必须记录新的 Run 和边界，不能因恢复了旧余额而省略。入桌前对账未成功时留在大厅重试，确认离桌后的最终对账完成或明确失败后才结束 Run。失败保留最后已知值并标记过期；缺失官方积分显示未知。旧记录中由余额计算的数值只能标记为历史估算，不能回填成曾经取得的官方积分。

当前 Run 的 Overview 积分和 Live 账户状态使用同一最新官方观测及更新时间；手动选择历史 Run 时保留历史统计并明确标注。默认选择跟随当前 Run，避免容器更新后仍看旧 Run。每个 Run 的净收益仍从零累计，赛季积分属于账户，不因新 Run 或筹码在账户与牌桌之间转移而人为归零。

本地不拿积分替代历史净收益，也不把 REST 在桌字段直接当成当前可下注筹码。官方合同没有承诺 `chips_at_table` 恒等于本次买入金额或 `stack + bet`，因此界面保留来源及其更新时间。支付账户的美元/USDC `balance` 与这里的虚拟筹码无关。上线验收将直接比较入桌前后的官方 `score`、后端保存值与公开展示，不只比较余额总和。
...
每手一个本地持久 session，同手不同动作有独立 decisionId 与 turn。完整原始上下文和历史持久保存；实际 Jev 请求使用精简投影，保留当前行动、最多 6 个同手先前选择及活跃对手证据，移除无关近期输赢和重复标识。独立慢循环增量整理每人最多 200 个已完成交手并发布快照；实时决策读取每手固定的已发布版本，不回填旧结算。结算、接收和发布时间均须满足该手的信息边界。只有完整匹配的行动元数据才用于价格/尺寸分母，不把未知当成零。公开摊牌是有偏样本，不是完整范围。组合策略的分析会话仍最多 12 次，每回合 4000 字符、合计 12,000 字符；完整文本另存。

此前 harness 版本为 `visible-context-v6`、`street-sized-raise-to-v2`、`opponent-encounters-v3` 与 `poker-harness-choice-v5`；双循环更新继续记录实际版本，并增加每手知识版本。所有改动保留版本、来源和信息截止，在冻结样本上比较行为与可靠性，并以新 Run 的真实净收益验证效果。均匀随机范围摊牌参考只保留用于审计，实际 Jev 请求移除该字段；它不等于对手下注范围胜率，历史收益不能套给替代行动。当前服务不在线自动改写策略。
...
纯 Jev 快路径不等待慢循环，也不调用研究 LLM。慢循环在独立工作线程中读取原始 SQLite，写入独立派生知识/审计数据库。`RESEARCH_ENABLED=true` 在真实服务中启用工作线程，`false` 关闭慢循环；Demo 始终不启动。`KNOWLEDGE_DATABASE_PATH` 默认为 `${DATABASE_PATH}.knowledge.sqlite`，必须与原始库分开。使用同一持久数据目录保留知识发布、游标和每手绑定。不要把知识库放在临时容器文件系统。

Live 下方展示 worker 是否运行、待处理手牌与审计、最近完成时间和发布版本。暂停/失败与积压不是零记录；先看 worker 状态，再检查实际 Jev 决策是否继续。决策详情在 Live 与 Replay 自动刷新，展示本手版本、发布与证据截止、准备/模型/记录/发送/确认耗时；知识读取是准备阶段子项，不能重复求和。

新的随机范围结果明确标记为异步事后审计，不属于当时模型输入。审计尚未完成或不可用不阻止出牌，旧历史缺失字段不补造。没有新的适用快照时使用已有知识或明确基础版本，不因此调用本地动作策略。每手固定版本跨重连和重启恢复，恢复不能采用该手之后才发布的知识。

`sh scripts/manage.sh backup` 分别创建原始库和已有派生库的一致 SQLite 快照；输出 `jev-*.sqlite` 与 `knowledge-*.sqlite`，均保留在私有备份目录。每份文件内部一致，两库不是跨库原子时间点；已固定知识本身保存在原始库。丢失派生结果可以按原始记录重新计算统计，但重新计算的发布时间不是过去发布时间；不能据此重建并替换旧手曾用的知识。实际验收和性能分布另记入 [verification.md](verification.md)，本文不宣称已达到延迟或盈利目标。
