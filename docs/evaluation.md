...
当前版本使用 `BOT_STRATEGY=jev` 与 [harness](harness.md)，补充牌型/听牌、位置、候选新增投入、跟注价格和长期对手证据，移除即时请求中无关近期输赢与重复标识。采用 Pi 风格请求投影，完整原始事件和冻结决策继续保存。`poker-cards` 的均匀随机范围摊牌参考带样本量及误差，仅保留在完整审计上下文，不进入实际 Jev 请求，也不能当作下注范围胜率或 EV。自动启动不限手数和时长，启用 auto-rebuy；调用无金额限制，每个提交动作仍须来自 Jev，最终失败记录并持久停牌。
...
新旧 harness 使用固定决策信息截止进行对照，记录输入版本、候选版本、选项差异、延迟、失败与费用；原候选重跑和重新生成候选的实验必须区分。不得将后来摊牌泄漏进原决策，也不得把原行动收益赋给替代行动。长期记忆只取结算及接收都早于 cutoff 的已完成交手；部分事件无法顺序对应时不捏造价格分母。当前没有自动在线学习。

## 纯 Jev 冻结样本复盘

2026-09-21 Run `2523bf71-54dd-47c2-a46f-1cad2d17f22f` 实际完成约 3 小时 15 分钟对局，295 手核实结算净 -10,551 筹码（-178.83 bb/100），30 手盈利、135 手亏损、130 手零收益。519 个 Jev 动作被接受，0 fallback；另有 2 条不可用决策记录。全部 295 手起始及最终筹码与原始事件逐手一致，这次亏损不能解释为旧版本地兜底或胜率显示算错。

21 次翻后高牌跟注投入 6,153 筹码，涉及 16 手的整手净收益为 -7,322。高牌可能有听牌，这个分类本身不证明跟注错误；整手亏损也不是改成弃牌能节省的金额。具体问题包括 A7 高牌河牌跟全下时将无法赢取的超额下注计入赔率，以及一次可免费 check 却 fold。新版本通过正确金额口径、牌型/听牌与行动原则提供依据，由 Jev 判断。

数据库另外有 3 个缺少 hero 底牌/起始筹码的观察记录：2 个桌上手牌已结束、1 个仍标记 playing，均不并入 295 手收益口径。上一轮 955 手含 fallback 的复盘是不同样本，见下文历史说明；两者不能相加后称为同一策略结果。

新版优先评估真实净筹码、bb/100、最大回撤、样本量和失败率。冻结状态下减少某类错误只能证明行为变化；盈利与策略优势需新的真实 Run 证据及足量样本，不能仅凭模型调用成功或少量盈利手宣称。
...
当前运行不使用 `TOTAL_BUDGET_USD`、`RUN_BUDGET_USD` 或 CLI `--budget-usd`。旧账本和未知用量继续保存，不能把未知预留当成已经扣费，也不清零记录以伪造可用额度。供应商实际拒绝、无效响应或超过期限时保存失败；Live 不生成本地行动。Jev 默认单次 10 秒、总期限 40 秒、首次调用后最多重试三次，实际仍受平台回合期限约束。失败停牌持久化，显式恢复见[运行手册](running.md#模型失败后的恢复)。

## 自动化验证范围

| 层       | 主要验证行为                                                                                           |
| -------- | ------------------------------------------------------------------------------------------------------ |
| Core     | Reducer、合法候选、raise-to 边界、信息截止、牌型/听牌/随机范围参考、对手统计和去重                     |
...
历史有界验收采用纯 Jev、单个 Bot 连接、独立本地验证数据库，计划最多 30 分钟并观察至少 10 手完成。第二次实际运行 24 分 49 秒，因开始服务器迁移而在手牌边界停止；不将其记作 30 分钟持续验收通过，实际结果见验证报告。当时核对已接受/拒绝/未决动作、旧版 fallback 原因、缺失历史、费用和正常离桌；该 fallback 行为不属于当前运行合同。真实运行期间不启动第二个同账号 WS 客户端。

断线、超时、ACK 丢失和恢复错误主要通过本地可控协议测试验证；自然运行中没有发生的故障，不宣称已经在生产环境实测。合成演示、接口探针与真实对局分别报告。工程可靠性与策略效果分别验收；短期输赢不能代替功能验收，盈利评估仍以真实净收益和足量样本为依据。
