...
`EvidenceBuilder` reads raw SQLite in read-only mode. It selects at most 100 verified completed live hands, creates separate opponent briefs and global leak reviews, and samples up to six examples round-robin across profitable, losing and zero-profit hands. Samples retain completion/receipt cutoffs and explicit missingness. Opponent keys are stable hashes of public names, not verified identities.
...
LLM_RESEARCH_LEAK_MIN_NEW_HANDS=100
