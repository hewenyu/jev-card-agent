...
右上角 GitHub 链接直达本仓库；Bot 正在真实牌桌对局且桌号已知时，Live 的 “Watch on OpenPoker” 打开该桌官方直播，换桌自动更新，停止或离桌时隐藏。
...
实时路径正常只请求一次 Jev；确定性统计和随机范围审计由独立慢工作线程处理，不串行等待额外 LLM。每手固定已发布的知识版本、证据截止和哈希，并跨重连/重启保留；当前牌面与行动继续更新。慢循环暂停或积压时使用已有可用知识或明确的基础版本。研究接口目前只生产确定性统计，不调用 LLM，也不能提交牌桌动作。
